...
namespace
{
	...
	surrogate16_type detect_surrogate_type(TCHAR ch)
	{
		...
		{
			if (ch < 0xDC00)
				return surrogate16_first_in_pair;
			return surrogate16_second_in_pair;
		}
	...
	CString convert_utf_to_string(const std::vector<utf16_character> &v) //
	{
		...
		for (int i = 0; i < (int)v.size(); ++i)
		{
			...
			else
			{
				...
				int vh = va >> 10;
				int vl = va & ((1 << 10) - 1);
				result += (TCHAR)(0xD800 + vh);
				result += (TCHAR)(0xDC00 + vl);
			}
		}
		return result;
	}
...
std::vector<CString> load_from_file(std::filesystem::path path)
{
	return process_and_filter_string([&] () {
		...
		if (in.fail())
			return result;
		auto length = (in.tellg() / sizeof(TCHAR));
		if (length < 1)
			return result;
		--length;
		in.seekg(2, std::ios::beg); // skip BOM
		in.read(reinterpret_cast<char *>(result.GetBuffer(static_cast<int>(length))), length * sizeof(TCHAR));
		result.ReleaseBuffer(static_cast<int>(length));
		return result;
	}());
}